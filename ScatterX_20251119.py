...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # 应用样式
        self.apply_styles()
        self.loglog_qmin.valueChanged.connect(self.update_loglog_range)
        self.loglog_qmax.valueChanged.connect(self.update_loglog_range)
    ...
    def create_tab2_layout(self):
        ...
        # -------------------------- LogLog 范围与结果（含分形维数） --------------------------
        group_layout.addWidget(QLabel("LogLog 范围:"), 3, 0)
        self.loglog_qmin = QDoubleSpinBox();  self.loglog_qmin.setMinimum(0); self.loglog_qmin.setMaximum(1000); self.loglog_qmin.setDecimals(3); self.loglog_qmin.setSingleStep(0.01)
        self.loglog_qmax = QDoubleSpinBox();  self.loglog_qmax.setMinimum(0); self.loglog_qmax.setMaximum(1000); self.loglog_qmax.setDecimals(3); self.loglog_qmax.setSingleStep(0.01)
        self.loglog_result = QLabel("斜率: --, 截距: --")
        self.loglog_dim   = QLabel("分形维数 D: --")
        ...
        self.loglog_fit_btn = QPushButton("LogLog 拟合")
        self.loglog_fit_btn.clicked.connect(lambda: self._fit_range('loglog', 1))
        group_layout.addWidget(self.loglog_fit_btn, 3, 5)
        group_layout.addWidget(self.loglog_dim, 3, 6, 1, 2)
    
    
    
        # 快速修复（放在 create_tab2_layout 里任意位置，只要晚于字典定义即可）
        for d in (self.region_masks, self.region_lines, self.fit_lines, self.fit_results):
            d['loglog'] = d.get('loglog', None if d is not self.fit_results else
                                {'slope': 0, 'intercept': 0, 'r_squared': 0, 'error': 0})    
        ...
        group_layout.addWidget(self.save_data_btn, 0, 8, 3, 1)
    ...
    # ===================== LogLog 范围选择 & 拟合 =====================
    def update_loglog_range(self):
        """LogLog 范围控件值改变时刷新图上的标记"""
        self._update_range('loglog', 1)
    
    def _fit_loglog(self):
        """对 LogLog 图执行 lnq~lnI 线性拟合并计算分形维数 D"""
        ...
        if qmin >= qmax:
            self.loglog_result.setText("错误: 起始值需小于结束值")
            return
    
        # 有效区间
        valid = (q >= qmin) & (q <= qmax) & (q > 0) & (I > 0)
        if np.count_nonzero(valid) < 2:
            self.loglog_result.setText("数据点不足，无法拟合")
            return
        ...
        self.loglog_result.setText(f"斜率: {slope:.4f}, 截距: {intercept:.4f}")
        # ===== 分形维数判定 =====
        slope_abs = abs(slope)
        if 3 < slope_abs < 4:
            D = 6 - slope_abs
        ...
        self.loglog_result.setText(f"斜率: {slope:.4f}, 截距: {intercept:.4f}")
        self.loglog_dim.setText(f"分形维数 D: {D:.4f} ({fractal_type})")
        ...
        # 清除旧拟合线（与 Guinier 等共用逻辑）
        if hasattr(self, '_loglog_fit_line') and self._loglog_fit_line:
            try:
                self._loglog_fit_line.remove()
            except:
                pass
        self._loglog_fit_line, = ax.plot(np.exp(x_line), np.exp(y_line), 'r-', lw=2)
    ...
    def save_analysis_results(self):
        ...
        try:
            with open(save_path, 'w', newline='', encoding='utf-8') as f:
                ...
                # ---------- 新增：分形维数结果 ----------
                # 从界面上实时读取
                loglog_text = self.loglog_dim.text()          # 例： "分形维数 D: 2.35 (质量分形)"
                if "D:" in loglog_text:
                    # 用正则提取数字和类型
                    import re
                    m = re.search(r"D:\s*([\d.]+)(?:\s*\((.+?)\))?", loglog_text)
                    if m:
                        D_val   = m.group(1)
                ...
                writer.writerow([])                       # 空行
                writer.writerow(["LogLog 分析"])
                writer.writerow(["分形维数 D", D_val])
                writer.writerow(["分形类型", frac_type])
                # 可选：把斜率/截距也再写一次
                slope_intercept = self.loglog_result.text()  # 例： "斜率: -3.234, 截距: 8.123"
    ...
    def _update_range(self, region, ax_index):
        ...
        if region == 'guinier':
            ...
            valid_data_idx = valid_idx & (q > 0) & (I > 0)
            x_display = np.log(q[valid_data_idx])
            y_display = np.log(I[valid_data_idx])
        
            # ✅ 使用 log(q) 作为 x 轴坐标
            log_q_min = np.log(qmin_widget.value())
            log_q_max = np.log(qmax_widget.value())
        
            self.region_lines[region][0] = ax.axvline(x=log_q_min, color='red', linestyle='--', alpha=0.5)
            self.region_lines[region][1] = ax.axvline(x=log_q_max, color='red', linestyle='--', alpha=0.5)
